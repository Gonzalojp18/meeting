/**
 * SEED SCRIPT - Upselling Combinations (CommonJS version)
 * 
...
// Schema inline para evitar problemas de import/export
const upsellingSchema = new mongoose.Schema(
    {
        name: { type: String, required: true, trim: true },
        type: { type: String, enum: ['upsell', 'cross-sell', 'combo', 'upgrade'], required: true },
        category: {
            type: String,
            enum: ['cafeteria', 'desayunos', 'hamburguesas', 'sandwiches', 'platos', 'ensaladas', 'wraps', 'tartas', 'bigtoast', 'entradas', 'postres', 'muffins', 'alfajores', 'licuados', 'detox', 'bebidas', 'combos-especiales'],
            required: true
        },
        triggerItemId: { type: mongoose.Schema.Types.ObjectId, default: null },
        triggerItemName: { type: String, trim: true, default: null },
        triggerCategoryId: { type: mongoose.Schema.Types.ObjectId, default: null },
        triggerCategoryName: { type: String, trim: true, default: null },
        suggestedItems: [{
            itemId: { type: mongoose.Schema.Types.ObjectId, required: true },
            itemName: { type: String, required: true, trim: true }
        }],
        copyText: { type: String, required: true, trim: true, maxlength: 100 },
        description: { type: String, trim: true, maxlength: 200 },
        ticketLevel: { type: String, enum: ['bajo', 'medio', 'alto'], default: 'medio' },
        timing: { type: String, enum: ['mañana', 'tarde', 'noche', 'todo-el-dia'], default: 'todo-el-dia' },
        daysActive: { type: [Number], default: [0, 1, 2, 3, 4, 5, 6] },
        displayLocations: {
            inMenu: { type: Boolean, default: false },
            inCheckout: { type: Boolean, default: true },
        ...
        metrics: {
            impressions: { total: { type: Number, default: 0 }, inMenu: { type: Number, default: 0 }, inCheckout: { type: Number, default: 0 }, inCart: { type: Number, default: 0 } },
            clicks: { total: { type: Number, default: 0 }, inMenu: { type: Number, default: 0 }, inCheckout: { type: Number, default: 0 }, inCart: { type: Number, default: 0 } },
...
const Upselling = mongoose.models.Upselling || mongoose.model('Upselling', upsellingSchema);

// Menu schema simplificado para lectura
const menuSchema = new mongoose.Schema({}, { strict: false });
const Menu = mongoose.models.Menu || mongoose.model('Menu', menuSchema);

// Helper para buscar producto por nombre
...
async function seedUpsellings() {
    try {
        ...
        console.log(`✅ Menú con ${menu.categories?.length || 0} categorías`);

        // Limpiar upsellings existentes
        console.log('🗑️  Limpiando upsellings...');
        ...
        for (const def of upsellingDefinitions) {
            ...
            if (def.trigger) {
                ...
                if (!triggerItem) {
                    console.log(`⚠️  Trigger no encontrado: "${def.trigger}"`);
                    skipped++;
                    continue;
                }
            }
            ...
            if (def.triggerCategory) {
                ...
                if (!triggerCategory) {
                    console.log(`⚠️  Categoría no encontrada: "${def.triggerCategory}"`);
                    skipped++;
                    continue;
                }
            }
            ...
            for (const name of def.suggested) {
                const item = findItem(menu, name);
                if (item) {
                    suggestedItems.push({ itemId: item.itemId, itemName: item.itemName });
                } else {
                    console.log(`⚠️  Sugerido no encontrado: "${name}"`);
                    allFound = false;
                }
            }
            ...
            upsellings.push({
                ...
                displayLocations: def.displayLocations || { inMenu: false, inCheckout: true, inCart: true },
                isActive: true,
        ...
        console.log('\n✅ SEED COMPLETADO!');
        console.log(`   ✓ Creados: ${upsellings.length}`);
        console.log(`   ✗ Omitidos: ${skipped}`);

        // Resumen por categoría
        console.log('\n📊 POR CATEGORÍA:');
        const cats = [...new Set(upsellings.map(u => u.category))];
        for (const cat of cats.sort()) {
            console.log(`   ${cat}: ${upsellings.filter(u => u.category === cat).length}`);
        }
