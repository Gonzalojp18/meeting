...
export function orderTemplate(payload) {
    ...
    // Etiqueta distintiva para B2B Ejecutivo
    if (data.orderMode === 'executive') {
        chunks.push(ESC_POS.TEXT_SIZE_NORMAL, ESC_POS.BOLD_ON);
        chunks.push(Buffer.from(`** MENÚ EJECUTIVO B2B **\n`));
    }

    chunks.push(ESC_POS.TEXT_SIZE_NORMAL, ESC_POS.BOLD_OFF);
    chunks.push(Buffer.from(`--------------------------------\n`));

    // ── Banner TAKEAWAY (solo Retiro en Sucursal) ──────────────────────────
    // Visibilidad máxima: primer dato que lee cocina → saben al instante
    // que va en envase para llevar, no se emplata.
    if (safeStr(data.deliveryMethod, '').toLowerCase().includes('retiro')) {
        chunks.push(ESC_POS.ALIGN_CENTER, ESC_POS.BOLD_ON, ESC_POS.TEXT_SIZE_LARGE);
        chunks.push(Buffer.from(`================================\n`));
        chunks.push(Buffer.from(`  *** TAKEAWAY ***  \n`));
        chunks.push(Buffer.from(`================================\n`));
        chunks.push(ESC_POS.TEXT_SIZE_NORMAL, ESC_POS.BOLD_OFF);
    }
    ...
    if (data.orderTiming === 'scheduled' && data.scheduledPickupAt) {
        const pickupDate = new Date(data.scheduledPickupAt);
        const pickupStr = pickupDate.toLocaleString('es-AR', {
            day: '2-digit', month: '2-digit', year: 'numeric',
            hour: '2-digit', minute: '2-digit'
        });
        chunks.push(ESC_POS.BOLD_ON);
        chunks.push(Buffer.from(`RETRO: ${pickupStr}\n`));
    ...
    items.forEach((item) => {
        ...
        if (Array.isArray(item?.customizations)) {
            item.customizations.forEach((c) => {
                const groupLabel = safeStr(c.groupName || c.group, '');
...
export function cashierTemplate(payload) {
    ...
    if (data.orderMode === 'executive') {
        chunks.push(ESC_POS.BOLD_ON);
        chunks.push(Buffer.from(`** B2B EJECUTIVO **\n`));
        chunks.push(ESC_POS.BOLD_OFF);
    }
    ...
    if (data.orderTiming === 'scheduled' && data.scheduledPickupAt) {
        ...
        chunks.push(ESC_POS.BOLD_ON);
        chunks.push(Buffer.from(`Retiro: ${pickupStr}\n`));
        chunks.push(ESC_POS.BOLD_OFF);
    }
