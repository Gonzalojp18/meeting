...
export async function executePrintSaga(orderOrId, metadata = { type: 'kitchen' }) {
    ...
    try {
        ...
        const printResults = await Promise.allSettled(
            targetPrinters.map(async (printer) => {
                try {
                    ...
                    const buffer = generateBuffer({ template: templateId, data: order, metadata });
        ...
        // Log to Print History (DB)
        if (!isMock) {
            // CAMBIO: Si es reimpresión, NO registrar nada en el historial por solicitud explícita.
            // "Manda bytes y olvida".
            if (metadata.isReprint) {
                console.log(`[SAGA] Reimpresión enviada a ${targetPrinters.map(p => p.name).join(', ')}. No se registra en historial.`);
